...
def inventory_list(request):
    ...
    sensors_count = InventoryItem.objects.filter(category='sensor').count()
    raw_materials_count = InventoryItem.objects.filter(category='raw_components').count()
...
def sensor_build(request):
    raw_materials = InventoryItem.objects.filter(category='raw_components')
    if request.method == 'POST':
        ...
        for component_id, quantity in zip(components, quantities):
            ...
            SensorComponent.objects.create(
                ...
                component=component,
                quantity_used=int(quantity)  # Store per-unit quantity, not total used
            )
        ...
        return redirect('inventory_list')  # Redirect to inventory list to see the new sensor
...
def sensor_detail(request, sensor_id):
    sensor = get_object_or_404(InventoryItem, id=sensor_id, category='sensor')
    ...
    if request.method == 'POST':
        ...
        if sensor.total_stock < quantity:
            return render(request, 'deployed_sensors.html', {
                ...
                'sensors': InventoryItem.objects.filter(category='sensor'),
